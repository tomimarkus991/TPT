...
namespace _18_09_20_ÜLDINE
{
    class Program
    {
        ...
        static void Alg_11(int suurus) // Paned numbri ülesse
        {
            int[] arvud = new int[suurus];
            ...
            for (int k = 0; k < suurus; k++)
                Console.Write(arvud[k] + " ");
            Console.WriteLine("Swap arv = {0}", swapn); // näitab vahetuste arvu
        ...
        static void Alg_13(int suurus)  // Teha array, kus on 40 elementi, mis võtab suvalisi arve suureuses kuni 0 - 50 ++ ning kõik arvud peavad olema unikaalsed
        {
            Random rnd = new Random();
            int[] arvud = new int[suurus];
            int uusArv;
            ...
            for (int k = 0; k <= suurus; k++)
            {
                uuesti:  // kui tuleb dubleeritud arv, siis tuleb uuesti siia ja kordab protsessi
                uusArv = rnd.Next(suurus);
